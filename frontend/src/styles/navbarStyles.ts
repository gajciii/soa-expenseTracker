...
export const navbarClasses = {
  ...
  linksContainer: 'flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 lg:gap-6 w-full sm:w-auto',
  linksWrapper: 'flex flex-col sm:flex-row items-center gap-3 sm:gap-4 lg:gap-6',
  link: (isActive: boolean) => `px-3 sm:px-4 py-2 rounded-lg transition-all duration-200 no-underline text-sm sm:text-base w-full sm:w-auto text-center sm:text-left ${isActive ? 'font-semibold' : 'font-normal hover:opacity-80'}`,
  iconLink: (isActive: boolean) => `p-2 rounded-lg transition-all duration-200 no-underline flex items-center justify-center ${isActive ? 'font-semibold' : 'font-normal hover:opacity-80'}`,
